...
def automate():
    ...
    for layer in range(layers + 1):  # Ensure it includes the final layer
        ...
        for turn in range(turns+1):
            ...
            if turn == 0:
              output_text.insert(tk.END, f"PTP {{X {points[0][0]}, Y {points[0][1]}, Z {points[0][2]}, A {a}, B {b}, C {c}, S 2., T 43.}}\n")

            if turn > 0 :
              points[0][1] -= e
              output_text.insert(tk.END, f"CIRC {{X {dummy_circ_x}, Y {dummy_circ_y}, Z {points[0][2]}}},{{X {points[0][0]}, Y {points[0][1]}, Z {points[0][2]}, A {a}, B {b}, C {c}}}\n")
                ...
                if k == "LIN":
                    if turn <= turns:
                        if i == 1:
                            ...
                            dummy_circ_x -= (0.6 * e)
                            dummy_circ_y -= (0.6 * e)
                        output_text.insert(tk.END, f"LIN {{X {points[i][0]}, Y {points[i][1]}, Z {points[i][2]}, A {a}, B {b}, C {c}}}\n")
                        i += 1
                    else:
                        output_text.insert(tk.END, f"LIN {{X {points[i][0]}, Y {points[i][1]}, Z {points[i][2]}, A {a}, B {b}, C {c}}}\n")
                    ...
                    if turn <= turns:
                        if j == 0:
                            ...
                            points_circ[j][1] += (e / 2)
                            points_circ[j + 1][0] -= e
                        output_text.insert(tk.END, f"CIRC {{X {points_circ[j][0]}, Y {points_circ[j][1]}, Z {points_circ[j][2]}}},{{X {points_circ[j + 1][0]}, Y {points_circ[j + 1][1]}, Z {points_circ[j + 1][2]}, A {a}, B {b}, C {c}}}\n")
                        j += 2
                    else:
                        output_text.insert(tk.END, f"CIRC {{X {points_circ[j][0]}, Y {points_circ[j][1]}, Z {points_circ[j][2]}}},{{X {points_circ[j + 1][0]}, Y {points_circ[j + 1][1]}, Z {points_circ[j + 1][2]}, A {a}, B {b}, C {c}}}\n")
              visualize_cuboid()
