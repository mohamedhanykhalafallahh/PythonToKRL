...
def add_point():
    ...
    if len(points+points_circ) <= 11:
            ...
            if len(points+points_circ) == 11:
                dummy_og_y= ((points[0][1] + points[4][1])/2)
...
def automate():
    ...
    for layer in range(layers + 1):  # Ensure it includes the final layer
        ...
        # Determine if this is an even layer (reverse order needed)
        is_even_layer = layer % 2 == 0
        
        # Store KRL commands for this layer to potentially reverse them
        layer_commands = []
        ...
        # Generate commands for all turns in this layer
        for turn in range(turns + 1):
            ...
            if turn == 0:
                cmd = f"PTP {{X {points[0][0]}, Y {points[0][1]}, Z {points[0][2]}, A {a}, B {b}, C {c}, S 2., T 43.}}\n"
                layer_commands.append(cmd)

            if turn > 0:
                points[0][1] -= e
                cmd = f"CIRC {{X {dummy_circ_x}, Y {dummy_circ_y}, Z {points[0][2]}}},{{X {points[0][0]}, Y {points[0][1]}, Z {points[0][2]}, A {a}, B {b}, C {c}}}\n"
                layer_commands.append(cmd)
                    ...
                    if k == "LIN":
                        if turn <= turns:
                            if i == 1:
                                ...
                                dummy_circ_x -= (0.6 * e)
                                dummy_circ_y -= (0.6 * e)
                            cmd = f"LIN {{X {points[i][0]}, Y {points[i][1]}, Z {points[i][2]}, A {a}, B {b}, C {c}}}\n"
                            layer_commands.append(cmd)
                            i += 1
                        else:
                            cmd = f"LIN {{X {points[i][0]}, Y {points[i][1]}, Z {points[i][2]}, A {a}, B {b}, C {c}}}\n"
                        ...
                        if turn <= turns:
                            if j == 0:
                                ...
                                points_circ[j][1] += (e / 2)
                                points_circ[j + 1][0] -= e
                            cmd = f"CIRC {{X {points_circ[j][0]}, Y {points_circ[j][1]}, Z {points_circ[j][2]}}},{{X {points_circ[j + 1][0]}, Y {points_circ[j + 1][1]}, Z {points_circ[j + 1][2]}, A {a}, B {b}, C {c}}}\n"
                            layer_commands.append(cmd)
                            j += 2
                        else:
                            cmd = f"CIRC {{X {points_circ[j][0]}, Y {points_circ[j][1]}, Z {points_circ[j][2]}}},{{X {points_circ[j + 1][0]}, Y {points_circ[j + 1][1]}, Z {points_circ[j + 1][2]}, A {a}, B {b}, C {c}}}\n"
        ...
        # Now output the layer commands in the correct order
        if is_even_layer and layer > 0:
            # For even layers (2, 4, 6, ...), reverse the order except for the first PTP command
            if layer_commands:
                # Keep the first PTP command at the beginning
                first_ptp = layer_commands[0]
                remaining_commands = layer_commands[1:]
                
                # Reverse the remaining commands
                remaining_commands.reverse()
                
                # Output in reversed order
                output_text.insert(tk.END, first_ptp)
                for cmd in remaining_commands:
                    output_text.insert(tk.END, cmd)
        else:
            # For odd layers (1, 3, 5, ...) or layer 0, use normal order
