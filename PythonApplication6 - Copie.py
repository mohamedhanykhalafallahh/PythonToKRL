...
# Global variables to track A-axis orientation for smooth transitions
previous_a_angle = 180.0  # Start with default A angle
...
def calculate_smooth_a_angle(dx, dy):
    """Calculate smooth A-angle transition from current position"""
    ...
    angle_diff = target_angle - current_a_angle
    ...
    # Limit the rotation to avoid large jumps (max 45 degrees per step for smoother motion)
    max_rotation = 45.0
    if abs(angle_diff) > max_rotation:
        angle_diff = max_rotation if angle_diff > 0 else -max_rotation
    
    # Update current A angle
    current_a_angle = normalize_angle(current_a_angle + angle_diff)
...
def generate_krl_code(points, motion_commands, points_circ, orientations):
    ...
    for k in motion_commands:
        if k == "LIN":
            ...
            # FDAT is set in the header, so no need to repeat it here
            
            # Calculate A angle based on TANGENT direction for pen-like movement
            # Get start point (previous point), middle point, and end point of CIRC
            if i > 0:  # If we have a previous point
                start_x = points[i-1][0]
                start_y = points[i-1][1]
            else:  # If this is the first CIRC, use the origin point
                start_x = points[0][0]
                start_y = points[0][1]
            ...
            krl_code += f"CIRC {{X {points_circ[j][0]}, Y {points_circ[j][1]}, Z {points_circ[j][2]}}},{{X {points_circ[j + 1][0]}, Y {points_circ[j + 1][1]}, Z {points_circ[j + 1][2]}, A {A_angle:.1f}, B 20.0, C 180.0}}\n"
            j += 2
            n += 2
    ...
    def ok_click():
        ...
        if middle_x and middle_y and middle_z and end_x and end_y and end_z:
            try:
                ...
                # Generate the KRL code with calculated A, constant B tilt (20°), and constant C (180°)
                krl_code = f"CIRC {{X {middle_x}, Y {middle_y}, Z {middle_z}}},{{X {end_x}, Y {end_y}, Z {end_z}, A {A_angle:.1f}, B 20.0, C 180.0}}\n"
            ...
            if turn_idx == 0:  # First turn in this layer
                if layer == 0:  # Very first turn of the entire sequence
                    ...
                    if tilt_along_travel_var.get():
                        # Reset A-angle for comfortable PTP start - use 0 degrees to avoid A4 issues
                        current_a_angle = 0.0
                        output_text.insert(tk.END, f"PTP {{X {turn_points[0][0]}, Y {turn_points[0][1]}, Z {turn_points[0][2]}, A 0.0, B {tilt_angle_deg}, C 180.0, S 2., T 43.}}\n")
                    else:
                        # Reset A-angle for comfortable PTP start - use 0 degrees to avoid A4 issues
                        current_a_angle = 0.0
                        output_text.insert(tk.END, f"PTP {{X {turn_points[0][0]}, Y {turn_points[0][1]}, Z {turn_points[0][2]}, A 0.0, B {b}, C {c}, S 2., T 43.}}\n")
...
def create_circ_transition(from_point, to_point, a, b, c, tilt_angle_deg):
    ...
    # Calculate A angle based on TANGENT direction for pen-like movement
    # Calculate direction vector from from_point to to_point
    dx = to_point[0] - from_point[0]
    dy = to_point[1] - from_point[1]
    ...
    # Generate the CIRC command with calculated A, constant B tilt (20°), and constant C (180°)
    output_text.insert(tk.END, f"CIRC {{X {intermediate_x}, Y {intermediate_y}, Z {intermediate_z}}},{{X {to_point[0]}, Y {to_point[1]}, Z {to_point[2]}, A {A_angle:.1f}, B 20.0, C 180.0}}\n")
        ...
        if k == "LIN":
            if tilt_along_travel_var.get():
                ...
                if i > 0:  # Make sure we have a previous point
                    ...
                    # Use smooth A-angle calculation to avoid large jumps
                    lin_a = calculate_smooth_a_angle(dx, dy)
                    
                    # Apply consistent tilt for pen-like movement
                    # Keep B-axis constant at tilt angle for all movements
                    lin_b = tilt_angle_deg  # Consistent B tilt for all movements
                    lin_c = 180.0  # Keep C constant for all movements
                else:
                ...
                # Normalize A angle to 0-360 range
                lin_a = lin_a % 360.0
            ...
            # Get start point (previous point), middle point, and end point of CIRC
            if i > 0:  # If we have a previous point
                start_x = turn_points[i-1][0]
                start_y = turn_points[i-1][1]
            else:  # If this is the first CIRC, use the origin point
                start_x = turn_points[0][0]
                start_y = turn_points[0][1]
            ...
            if tilt_along_travel_var.get():
                # Use calculated A, constant B tilt (20°), and constant C (180°)
                output_text.insert(tk.END, f"CIRC {{X {turn_points_circ[j][0]}, Y {turn_points_circ[j][1]}, Z {turn_points_circ[j][2]}}},{{X {turn_points_circ[j + 1][0]}, Y {turn_points_circ[j + 1][1]}, Z {turn_points_circ[j + 1][2]}, A {A_angle:.1f}, B 20.0, C 180.0}}\n")
            else:
                # Use calculated A, constant B tilt (20°), and constant C (180°)
                output_text.insert(tk.END, f"CIRC {{X {turn_points_circ[j][0]}, Y {turn_points_circ[j][1]}, Z {turn_points_circ[j][2]}}},{{X {turn_points_circ[j + 1][0]}, Y {turn_points_circ[j + 1][1]}, Z {turn_points_circ[j + 1][2]}, A {A_angle:.1f}, B 20.0, C 180.0}}\n")
            j += 2
...
def clear_for_auto():
    global krl_counter, current_a_angle
    _push_undo_snapshot()
    krl_counter = 1
...
def clear_all():
    global points, points_circ, motion_commands, krl_counter, current_a_angle
    _push_undo_snapshot()
    points = []
    points_circ = []
    motion_commands = []
...
BASE_DATA[1] = {{X 0.0, Y 0.0, Z 100.0, A 0.0, B 0.0, C 0.0}}
$BASE = BASE_DATA[1]
...
def open_generate_popup():
    ...
    zr_entry.insert(0, "-50")  # Safe working height above singularity region
...
default_increment = 0
default_ori = 180
tool_z = -50  # Safe working height above singularity region
